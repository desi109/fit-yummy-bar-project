...
function pessimisticUpdate(opts) {
    return (source) => {
        return source.pipe(mapActionAndState(), operators_1.concatMap(runWithErrorHandling(opts.run, opts.onError)));
    };
}
exports.pessimisticUpdate = pessimisticUpdate;
function optimisticUpdate(opts) {
    return (source) => {
        return source.pipe(mapActionAndState(), operators_1.concatMap(runWithErrorHandling(opts.run, opts.undoAction)));
    };
}
...
function fetch(opts) {
    return (source) => {
        if (opts.id) {
            const groupedFetches = source.pipe(mapActionAndState(), operators_1.groupBy(([action, store]) => {
                return opts.id(action, store);
            }));
            return groupedFetches.pipe(operators_1.mergeMap((pairs) => pairs.pipe(operators_1.switchMap(runWithErrorHandling(opts.run, opts.onError)))));
        }
        return source.pipe(mapActionAndState(), operators_1.concatMap(runWithErrorHandling(opts.run, opts.onError)));
    };
}
...
function navigation(component, opts) {
    return (source) => {
        const nav = source.pipe(mapActionAndState(), operators_1.filter(([action, state]) => isStateSnapshot(action)), operators_1.map(([action, state]) => {
        ...
        }), operators_1.filter(([snapshot, state]) => !!snapshot));
        return nav.pipe(operators_1.switchMap(runWithErrorHandling(opts.run, opts.onError)));
    };
}
...
function runWithErrorHandling(run, onError) {
    return ([action, state]) => {
        try {
            ...
            return r.pipe(operators_1.catchError((e) => wrapIntoObservable(onError(action, e))));
        }
        catch (e) {
...
function mapActionAndState() {
    return (source) => {
        return source.pipe(operators_1.map((value) => {
...
let DataPersistence = class DataPersistence {
    ...
    pessimisticUpdate(actionType, opts) {
        return this.actions.pipe(effects_1.ofType(actionType), operators_1.withLatestFrom(this.store), pessimisticUpdate(opts));
    }
    /**
     *
    ...
    optimisticUpdate(actionType, opts) {
        return this.actions.pipe(effects_1.ofType(actionType), operators_1.withLatestFrom(this.store), optimisticUpdate(opts));
    }
    /**
     *
    ...
    fetch(actionType, opts) {
        return this.actions.pipe(effects_1.ofType(actionType), operators_1.withLatestFrom(this.store), fetch(opts));
    }
    /**
    ...
    navigation(component, opts) {
        return this.actions.pipe(operators_1.withLatestFrom(this.store), navigation(component, opts));
    }
};
