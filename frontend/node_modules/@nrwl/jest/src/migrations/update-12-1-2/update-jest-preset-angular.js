...
function updateJestConfig(tree) {
    executor_options_utils_1.forEachExecutorOptions(tree, '@nrwl/jest:jest', (options, projectName) => {
        const config = require_jest_config_1.getJestObject(path_1.join(tree.root, options.jestConfig));
        // migrate serializers
        if (config.snapshotSerializers &&
            ...
            try {
                update_config_1.removePropertyFromJestConfig(tree, options.jestConfig, 'snapshotSerializers');
                update_config_1.addPropertyToJestConfig(tree, options.jestConfig, 'snapshotSerializers', snapshotSerializers);
            }
            catch (_a) {
                devkit_1.logger.error(devkit_1.stripIndents `Unable to update snapshotSerializers for project ${projectName}.
        ...
        try {
            ...
            const setupTestPath = path_1.join(sourceRoot, 'test-setup.ts');
            ...
            devkit_1.logger.error(devkit_1.stripIndents `Unable to update test-setup.ts for project ${projectName}.`);
        }
    });
}
