...
function projectsToHtml(projects, graph, affected, focus, groupByFolder, exclude, layout) {
    let f = fs_1.readFileSync(path_1.join(__dirname, '../core/dep-graph/index.html')).toString();
    f = f
...
function generateGraph(args, affectedProjects) {
    ...
    if (args.file === undefined || args.file.endsWith('html')) {
        html = projectsToHtml(projects, graph, affectedProjects, args.focus || null, args.groupByFolder || false, args.exclude || [], layout);
    }
    else {
    ...
    if (args.file) {
        ...
        if (ext === 'html') {
            ...
            filename = `${folder}/${filename}`;
            fs_extra_1.ensureDirSync(path_1.dirname(filename));
