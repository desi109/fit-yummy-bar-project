    ...
    assets.forEach((asset) => {
        if (typeof asset === 'string') {
            globbedFiles(asset, rootDir).forEach((globbedFile) => {
                files.push({
                    input: path_1.join(rootDir, globbedFile),
                    output: path_1.join(outDir, path_1.basename(globbedFile)),
                });
            });
        }
        else {
            globbedFiles(asset.glob, path_1.join(rootDir, asset.input), asset.ignore).forEach((globbedFile) => {
                files.push({
                    input: path_1.join(rootDir, asset.input, globbedFile),
                    output: path_1.join(outDir, asset.output, globbedFile),
                });
            });
        }
    });
...
function copyAssetFiles(files) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield Promise.all(files.map((file) => fs_extra_1.copy(file.input, file.output)));
