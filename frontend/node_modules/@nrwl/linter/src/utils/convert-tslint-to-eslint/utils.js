...
function ensureESLintPluginsAreInstalled(host, eslintPluginsToBeInstalled) {
    ...
    return devkit_1.addDependenciesToPackageJson(host, {}, additionalDevDependencies);
}
...
function convertTSLintConfig(rawTSLintJson, tslintJsonPath, ignoreExtendsVals) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        const convertedProject = yield convert_to_eslint_config_1.convertToESLintConfig(tslintJsonPath, rawTSLintJson, ignoreExtendsVals);
        convertedProject.convertedESLintConfig.rules =
            convertedProject.convertedESLintConfig.rules || {};
        /**
         * Apply the custom converter for the nx-module-boundaries rule if applicable
         */
        const convertedNxRule = convert_nx_enforce_module_boundaries_rule_1.convertTslintNxRuleToEslintNxRule(rawTSLintJson);
        if (convertedNxRule) {
...
function deduplicateOverrides(overrides = []) {
    ...
    for (const override of overrides) {
        const mapKey = typeof override.files === 'string'
            ? override.files
            : override.files.join(',');
    ...
    for (const [, overrides] of map.entries()) {
        ...
        for (const override of overridesArr) {
            mergedOverride = Object.assign(Object.assign({}, mergedOverride), override);
        }
